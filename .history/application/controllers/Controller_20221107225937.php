...
class Controller extends CI_Controller
{
  ...
  public function sendKomentar_process($idPertanyaan)
  {
    ...
    if ($this->form_validation->run() == true) {
      //Mengambil data masukan pengguna
      $namaAlias = $this->session->userdata('nama');
      $deskripsiKomentar = $this->input->post('deskripsiKomentar');
      $waktuKirim = $this->input->post('waktuKirim');
      //Menyimpan data masukan pengguna dan menampilkan pesan berhasil
      if ($this->session->userdata('hakAkses') == 'siswa') {
        $this->Pertanyaan->saveKomentarSiswa($idPertanyaan, $namaAlias, $deskripsiKomentar, $waktuKirim);
        $this->session->set_flashdata('success_register', 'Komentar Berhasil Dikirimkan');
        echo base_url('pertanyaan/'. $idPertanyaan);
      } else {
        $this->Pertanyaan->saveKomentarGuru($idPertanyaan, $namaAlias, $deskripsiKomentar, $waktuKirim);
        $this->session->set_flashdata('success_register', 'Komentar Berhasil Dikirimkan');
        echo base_url('pertanyaan/'. $idPertanyaan);
      }
    } else {
