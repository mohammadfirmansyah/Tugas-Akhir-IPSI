  ...
  public function sendKomentar_process($idPertanyaan)
  {
    ...
    if ($this->form_validation->run() == true) {
      ...
      if ($this->session->userdata('hakAkses') == 'siswa') {
        ...
        $this->Pertanyaan->saveKomentarSiswa($idPertanyaan, $idSiswa, $deskripsiKomentar, $waktuKirim);
        $this->session->set_flashdata('success_register', 'Komentar Berhasil Dikirimkan');
        echo base_url('pertanyaan/'. $idPertanyaan);
      } else {
        $namaGuru = $this->session->userdata('nama');
        $this->Pertanyaan->saveKomentarGuru($idPertanyaan, $idGuru, $deskripsiKomentar, $waktuKirim);
        $this->session->set_flashdata('success_register', 'Komentar Berhasil Dikirimkan');
        echo base_url('pertanyaan/'. $idPertanyaan);
      }
    } else {
