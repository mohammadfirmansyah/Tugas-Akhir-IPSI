...
class Controller extends CI_Controller
{
  ...
  public function registerGuru_process()
  {
    ...
    if ($this->form_validation->run() == true) {
      ...
      if ($this->Pengguna->searchEmail($email) == false) {
  ...
  public function sendKomentar_process($idPertanyaan)
  {
    ...
    if ($this->form_validation->run() == true) {
      ...
      if ($this->session->userdata('hakAkses') == 'siswa') {
        ...
        $this->Pertanyaan->saveKomentars($idPertanyaan, $namaSiswa, $deskripsiKomentar, $jenisKomentar, $waktuKirim);
  ...
  public function addKelas_process()
  {
    ...
    if ($this->form_validation->run() == true) {
      ...
      redirect('controller/addKelas_database_validation/'. $kodeKelasFix.'/'. $mataPelajaran);
    } else {
      //Data masukan pengguna belum lengkap
      $this->session->set_flashdata('error', '
      <div class="alert alert-danger" role="alert" style="text-align: center;">
  ...
  public function addKelas_database_validation($kodeKelasFix, $mataPelajaran){
    $email = $this->session->userdata('email');
    $idGuru = $this->Pengguna->getIdByEmailGuru($email);
    $namaGuru = $this->session->userdata('nama');
    $idKelas = $this->Kelas->getIdKelasByKode($kodeKelasFix);
