        ...
        <ul>
          <li>
            <h1 class="logo me-auto">
              <a href="<?php echo base_url(''); ?>" class="logo me-auto"><img src="assets/img/logo-alternative.png" alt="" class="img-fluid"></a>
            </h1>
          </li>
          ...
          <li class="dropdown"><a href="<?php echo base_url('kelasSaya'); ?>"><span>Kelas Saya</span> <i class="bi bi-chevron-down"></i></a>
            <ul>
              ...
              ?>
                <li><a href="<?php echo base_url('addKelas'); ?>">Buat Kelas Baru Sekarang</a></li>
              <?php } ?>
            </ul>
          </li>
      </nav> <!-- .navbar -->
      ...
      <div class="card-deck-wrapper">
        <div class="card-deck">
          <?php if (empty($List_Kelas)) { ?>
            <h1 style="font-size: 32px; text-align: center;">Anda belum membuat kelas!</h1>
            <h2 style="text-align: center;" class="mb-4">Buat kelas sekarang dengan menekan tombol dibawah ini.</h2>
            <a style="text-align: center; margin-left: auto; margin-right: auto; display: grid; max-width: 250px;" class="btn-get-started_i mb-5" href="<?php echo base_url('addKelas'); ?>">TAMBAH KELAS +</a>
          <?php } else if ((!empty($List_Kelas))) { ?>
            <?php foreach ($idKelas as $idKelasGuru) :
              $List_Pertanyaan = $this->Pertanyaan->getAllPertanyaanGuru($idKelasGuru->idKelas);
            ?>
            <?php } else if ((!empty($List_Pertanyaan))) { ?>
            <h1 style="font-size: 32px; text-align: center;">Siswa anda belum menyampaikan pertanyaan!</h1>
            <h2 style="text-align: center;" class="mb-4">Bagikan kode kelas kepada siswa anda agar mereka bisa memberikan pertanyaan.</h2>
            <a style="text-align: center; margin-left: auto; margin-right: auto; display: grid; max-width: 200px;" class="btn-get-started_i mb-5" href="<?php echo base_url('kelasSaya'); ?>">KELAS SAYA</a>
            <?php endforeach; ?>
          <?php } else { ?>
            <?php foreach ($idKelas as $idKelasGuru) :
              $List_Pertanyaan = $this->Pertanyaan->getAllPertanyaanGuru($idKelasGuru->idKelas);
